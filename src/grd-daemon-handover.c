                         ...
                         gpointer      user_data)
{
  ...
  GrdDBusRemoteDesktopHandover *proxy;
  GSocketConnection *socket_connection;
  GrdDaemonHandover *daemon_handover;
  GrdRdpServer *rdp_server;

  proxy = GRD_DBUS_REMOTE_DESKTOP_HANDOVER (object);
  if (!grd_dbus_remote_desktop_handover_call_take_client_finish (
...
static void
on_take_client_ready (GrdDBusRemoteDesktopHandover *interface,
                      GrdDaemonHandover            *daemon_handover)
{
  ...
  grd_dbus_remote_desktop_handover_call_take_client (interface,
                                                     NULL,
                                                     cancellable,
                            ...
                            gpointer      user_data)
{
  ...
  proxy = GRD_DBUS_REMOTE_DESKTOP_HANDOVER (object);
  if (!grd_dbus_remote_desktop_handover_call_start_handover_finish (
                ...
                const char        *password)
{
  ...
  grd_dbus_remote_desktop_handover_call_start_handover (
...
static void
on_remote_desktop_handover_proxy_acquired (GObject      *object,
                                           GAsyncResult *result,
                                           gpointer      user_data)
{
  GrdDaemonHandover *daemon_handover = user_data;
  g_autoptr (GrdDBusRemoteDesktopHandover) proxy;
  g_autoptr (GError) error = NULL;

  proxy = grd_dbus_remote_desktop_handover_proxy_new_for_bus_finish (result,
                                                                     &error);
  if (!proxy)
    {
...
static void
on_remote_desktop_dispatcher_handover_requested (GObject      *object,
                                                 GAsyncResult *result,
                                                 gpointer      user_data)
{
  GrdDaemonHandover *daemon_handover = user_data;
  ...
  success = grd_dbus_remote_desktop_dispatcher_call_request_handover_finish (
  ...
  grd_dbus_remote_desktop_handover_proxy_new_for_bus (
    G_BUS_TYPE_SYSTEM,
    G_DBUS_PROXY_FLAGS_NONE,
    REMOTE_DESKTOP_BUS_NAME,
    object_path,
    cancellable,
    on_remote_desktop_handover_proxy_acquired,
    daemon_handover);
}

static void
on_remote_desktop_dispatcher_proxy_acquired (GObject      *object,
                                             GAsyncResult *result,
                                             gpointer      user_data)
{
  GrdDaemonHandover *daemon_handover = user_data;
  GCancellable *cancellable =
    grd_daemon_get_cancellable (GRD_DAEMON (daemon_handover));
  g_autoptr (GrdDBusRemoteDesktopDispatcher) proxy = NULL;
  g_autoptr (GError) error = NULL;

  proxy = grd_dbus_remote_desktop_dispatcher_proxy_new_for_bus_finish (result,
                                                                       &error);
  if (!proxy)
    {
  ...
  grd_dbus_remote_desktop_dispatcher_call_request_handover (
    daemon_handover->remote_desktop_dispatcher,
    cancellable,
    on_remote_desktop_dispatcher_handover_requested,
    daemon_handover);
}
...
grd_daemon_handover_startup (GApplication *app)
{
  ...
  grd_dbus_remote_desktop_dispatcher_proxy_new_for_bus (
    G_BUS_TYPE_SYSTEM,
    G_DBUS_PROXY_FLAGS_DO_NOT_AUTO_START,
    REMOTE_DESKTOP_BUS_NAME,
    REMOTE_DESKTOP_OBJECT_PATH,
    cancellable,
    on_remote_desktop_dispatcher_proxy_acquired,
    daemon_handover);
