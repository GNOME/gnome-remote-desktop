...
grd_credentials_one_time_lookup (GrdCredentials      *credentials,
                                 ...
                                 GError             **error)
{
  GrdCredentialsOneTime *credentials_one_time = GRD_CREDENTIALS_ONE_TIME (credentials);
  ...
  switch (type)
    ...
    case GRD_CREDENTIALS_TYPE_RDP:
      ...
      g_variant_builder_add (&builder, "{sv}",
                             "username", g_variant_new_string (
                             credentials_one_time->rdp.username));
      g_variant_builder_add (&builder, "{sv}",
                             "password", g_variant_new_string (
                             credentials_one_time->rdp.password));
